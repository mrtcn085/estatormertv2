...
function matchTable(rows){ return `<div class="table-wrap"><table class="table"><thead><tr><th>Skor</th><th>Alıcı</th><th>İlan/Portföy</th><th>Fiyat</th><th>Emsal</th><th>Komisyon</th><th>WhatsApp</th></tr></thead><tbody>${rows.map(r=>{const c=commission(r.a); return `<tr><td><b>${r.m.score}</b><br><span class="muted">${esc(r.m.reasons.join(', '))}</span></td><td>${esc(r.k.ad_soyad)}<br>${esc(formatTRPhone(r.k.telefon)||'')}</td><td>${esc(r.a.baslik||r.a.url||'-')}<br><span class="muted">${esc([r.a.sehir,r.a.ilce,r.a.mahalle].filter(Boolean).join(' / '))}</span></td><td>${money(estatePrice(r.a))}</td><td>${compHtml(r.a)}</td><td>${esc(c.text)}</td><td>${waHtml(r.k.telefon)}</td></tr>`}).join('')}</tbody></table></div>` }
...
function fileLinks(p){ const links=[]; if(p.sozlesme_url) links.push(`<a target="_blank" href="${esc(p.sozlesme_url)}">📄 Sözleşme</a>`); if(p.tapu_url) links.push(`<a target="_blank" href="${esc(p.tapu_url)}">🧾 Tapu</a>`); return links.join(' · '); }
function tablePortfoy(){ return `<div class="table-wrap"><table class="table"><thead><tr><th>Görsel</th><th>Portföy</th><th>Fiyat</th><th>Satıcı</th><th>Evrak</th><th></th></tr></thead><tbody>${cache.portfoyler.map(p=>{const c=commission(p);const photo=parseFirstPhoto(p.foto_urls);return `<tr><td>${photo?`<img class="thumb" src="${esc(photo)}">`:'-'}</td><td><b>${esc(p.baslik)}</b><br><span class="muted">${esc([p.tur,p.alt_tur,p.islem_tipi,p.sehir,p.ilce,p.mahalle,p.oda].filter(Boolean).join(' / '))}</span><br><span class="muted">${p.kiracili?'Kiracılı: Evet':''} ${parseArr(p.imar_durumlari).length?'İmar: '+parseArr(p.imar_durumlari).join(', '):''}</span></td><td>${money(p.fiyat)}<br>${compHtml(p)}<br><span class="muted">${esc(c.text)}</span></td><td>${esc(p.satici_ad_soyad||'-')}<br>${esc(formatTRPhone(p.satici_telefon)||'')}<br><span class="muted">TC: ${esc(p.satici_tc||'-')}</span><br>${waHtml(p.satici_telefon)}</td><td>${fileLinks(p)||'-'}</td><td><button class="btn small" onclick="goMatchAsset('${p.id}','portfoy')">Alıcı bul</button> <button class="btn small danger" onclick="del('portfoyler','${p.id}')">Sil</button></td></tr>`}).join('')}</tbody></table></div>` }
